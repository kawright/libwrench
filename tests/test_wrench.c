...
int main(){
    ...
    wr_Test test_ASSERTSTRGTEQ_a_gt_b                           = NULL;
    wr_Test test_ASSERTSTRGTEQ_a_equals_b                       = NULL;
    wr_Test test_ASSERTSTRGTEQ_a_lt_b                           = NULL;
    wr_Test test_ASSERTSTRGTEQ_empty_strs                       = NULL;
    wr_Test test_ASSERTSTRGTEQ_a_empty                          = NULL;
    wr_Test test_ASSERTSTRGTEQ_b_empty                          = NULL;
    wr_Test test_ASSERTSTRGTEQ_a_substr_of_b                    = NULL;
    wr_Test test_ASSERTSTRGTEQ_b_substr_of_a                    = NULL;
    wr_Test test_ASSERTSTRLT_a_gt_b                             = NULL;
    wr_Test test_ASSERTSTRLT_a_equals_b                         = NULL;
    wr_Test test_ASSERTSTRLT_a_lt_b                             = NULL;
    wr_Test test_ASSERTSTRLT_empty_strs                         = NULL;
    wr_Test test_ASSERTSTRLT_a_empty                            = NULL;
    wr_Test test_ASSERTSTRLT_b_empty                            = NULL;
    wr_Test test_ASSERTSTRLT_a_substr_of_b                      = NULL;
    wr_Test test_ASSERTSTRLT_b_substr_of_a                      = NULL;
    wr_Test test_ASSERTSTRLTEQ_a_gt_b                           = NULL;
    wr_Test test_ASSERTSTRLTEQ_a_equals_b                       = NULL;
    wr_Test test_ASSERTSTRLTEQ_a_lt_b                           = NULL;
    wr_Test test_ASSERTSTRLTEQ_empty_strs                       = NULL;
    wr_Test test_ASSERTSTRLTEQ_a_empty                          = NULL;
    ...
    err = wr_newtest(&cb_ASSERTSTRGTEQ_a_gt_b,
        "Test wr_ASSERTSTRGTEQ where `a` is greater than `b`",
        &test_ASSERTSTRGTEQ_a_gt_b);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRGTEQ_a_equals_b,
        "Test wr_ASSERTSTRGTEQ where `a` is equal to `b`",
        &test_ASSERTSTRGTEQ_a_equals_b);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRGTEQ_a_lt_b,
        "Test wr_ASSERTSTRGTEQ where `a` is less than `b`",
        &test_ASSERTSTRGTEQ_a_lt_b);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRGTEQ_empty_strs,
        "Test wr_ASSERTSTRGTEQ with two empty strings",
        &test_ASSERTSTRGTEQ_empty_strs);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRGTEQ_a_empty,
        "Test wr_ASSERTSTRGTEQ where `a` is an empty string",
        &test_ASSERTSTRGTEQ_a_empty);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRGTEQ_b_empty,
        "Test wr_ASSERTSTRGTEQ where `b` is an empty string",
        &test_ASSERTSTRGTEQ_b_empty);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRGTEQ_a_substr_of_b,
        "Test wr_ASSERTSTRGTEQ where `b` begins with the substring `a`",
        &test_ASSERTSTRGTEQ_a_substr_of_b);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRGTEQ_b_substr_of_a,
        "Test wr_ASSERTSTRGTEQ where `a` begins with the substring `b`",
        &test_ASSERTSTRGTEQ_b_substr_of_a);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLT_a_gt_b,
        "Test wr_ASSERTSTRLT where `a` is greater than `b`",
        &test_ASSERTSTRLT_a_gt_b);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLT_a_equals_b,
        "Test wr_ASSERTSTRLT where `a` is equal to `b`",
        &test_ASSERTSTRLT_a_equals_b);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLT_a_lt_b,
        "Test wr_ASSERTSTRLT where `a` is less than `b`",
        &test_ASSERTSTRLT_a_lt_b);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLT_empty_strs,
        "Test wr_ASSERTSTRLT with two empty strings",
        &test_ASSERTSTRLT_empty_strs);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLT_a_empty,
        "Test wr_ASSERTSTRLT where `a` is an empty string",
        &test_ASSERTSTRLT_a_empty);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLT_b_empty,
        "Test wr_ASSERTSTRLT where `b` is an empty string",
        &test_ASSERTSTRLT_b_empty);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLT_a_substr_of_b,
        "Test wr_ASSERTSTRLT where `b` begins with the substring `a`",
        &test_ASSERTSTRLT_a_substr_of_b);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLT_b_substr_of_a,
        "Test wr_ASSERTSTRLT where `a` begins with the substring `b`",
        &test_ASSERTSTRLT_b_substr_of_a);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLTEQ_a_gt_b,
        "Test wr_ASSERTSTRLTEQ where `a` is greater than `b`",
        &test_ASSERTSTRLTEQ_a_gt_b);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLTEQ_a_equals_b,
        "Test wr_ASSERTSTRLTEQ where `a` is equal to `b`",
        &test_ASSERTSTRLTEQ_a_equals_b);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLTEQ_a_lt_b,
        "Test wr_ASSERTSTRLTEQ where `a` is less than `b`",
        &test_ASSERTSTRLTEQ_a_lt_b);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLTEQ_empty_strs,
        "Test wr_ASSERTSTRLTEQ with two empty strings",
        &test_ASSERTSTRLTEQ_empty_strs);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLTEQ_a_empty,
        "Test wr_ASSERTSTRLTEQ where `a` is an empty string",
        &test_ASSERTSTRLTEQ_a_empty);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLTEQ_b_empty,
        "Test wr_ASSERTSTRLTEQ where `b` is an empty string",
        &test_ASSERTSTRLTEQ_b_empty);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLTEQ_a_substr_of_b,
        "Test wr_ASSERTSTRLTEQ where `b` begins with the substring `a`",
        &test_ASSERTSTRLTEQ_a_substr_of_b);
    assert(err == wr_ERROK);

    err = wr_newtest(&cb_ASSERTSTRLTEQ_b_substr_of_a,
        "Test wr_ASSERTSTRLTEQ where `a` begins with the substring `b`",
